...
class Movie():
    """This is a class to store movie information,
     title, story, poster, trailer, ratings...
    """
    # will be added to __doc__ automatically
    # others are __name__ __module__ __dict__
...
class TVShow(Movie):  # Episodes are a nother kind os movie
    ...
    ''' To store tv show information, title, season,
     episode, trailer, tv station ...
    '''

    def __init__(self, title, story, poster, trailer, season, episode, station, ended):

        Movie.__init__(self, title, story, poster, trailer)
        # super(self.__class__, self).__init__(title, story, poster, trailer)  # using super 
        self.season = season
